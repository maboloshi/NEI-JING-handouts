...
\newenvironment{jiaozhu}[1][【校注】]{%
  ...
  \begin{enumerate}[%
    %label=\mylabel{\arabic*}{\circledctr*},before=\small,fullwidth,    % 带圈数字 列标
    label=\mylabel{\arabic*}{\arabic*.},before=\small,fullwidth,        % 数字 列标
...
\protected\def\myjzref#1{% 注解和正文中的引用(引用到原文)
  %\hyperlink{\theyuanwen:#1}{\circlednum{#1}}}               % 带圈数字
  \hyperlink{\theyuanwen:#1}{\textsuperscript{[#1]}}}         % 带[]数字

%\def\sb#1{\myref{#1}{\textsuperscript{\circlednum{#1}}}}    % 带圈数字上标
\def\sb#1{\myref{#1}{\textsuperscript{[#1]}}}                % []上标
